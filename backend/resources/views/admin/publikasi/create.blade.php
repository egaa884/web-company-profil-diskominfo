...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Publikasi Baru</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Form Publikasi</h6>
        </div>
        <div class="card-body">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="kategori" id="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                                ...
                                <option value="pengaduan" {{ old('kategori') == 'pengaduan' ? 'selected' : '' }}>Laporan Pengaduan</option>
                                <option value="penerima" {{ old('kategori') == 'penerima' ? 'selected' : '' }}>Laporan Penerimaan</option>
                                <option value="surveikepuasan" {{ old('kategori') == 'surveikepuasan' ? 'selected' : '' }}>Survei Kepuasan Masyarakat</option>
                                <option value="surveievaluasi" {{ old('kategori') == 'surveievaluasi' ? 'selected' : '' }}>Survei Evaluasi Pelayanan Publik</option>
                            </select>
                            @error('kategori')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="ringkasan" id="ringkasan" class="form-control @error('ringkasan') is-invalid @enderror" 
                                      rows="3" placeholder="Masukkan ringkasan singkat publikasi">{{ old('ringkasan') }}</textarea>
                            @error('ringkasan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="isi" id="isi" class="form-control @error('isi') is-invalid @enderror" 
                                      rows="6" placeholder="Masukkan konten lengkap publikasi">{{ old('isi') }}</textarea>
                            @error('isi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="card bg-light">
                            <div class="card-body">
                                ...
                                <small class="text-muted">
                                    <strong>Slug:</strong> Akan dibuat otomatis dari judul<br>
                                    <strong>Dibuat:</strong> {{ now()->format('d/m/Y H:i') }}<br>
                                    <strong>Status:</strong> <span id="status-display">Draft</span>
                                </small>
                            </div>
                        </div>
                    </div>
...
// Auto-generate slug from judul (optional, for preview)
